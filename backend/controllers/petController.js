...
const createPet = async (req, res) => {
    const { name, description, photo, breed, species } = req.body;
    try {
        const petExists = await User.findOne({ name });
        if (petExists) return res.status(400).json({ message: 'Pet already exists' });

        const pet = await User.create({ name, description, photo, breed, species });
        res.status(201).json({ id: pet.id, name: pet.name, description: pet.description, photo: pet.photo, breed: pet.breed, species: pet.species });
    } catch (error) {
